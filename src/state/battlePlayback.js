 ...
 *
 * @param {{attackerCountry: string, defenderCountry: string, attackerColour?: string,
 *          territoryId: string, territoryName: string, startingAttackers: number[],
 *          startingDefenders: number[], records: object[], state: string,
 *          tookTerritory: boolean}} battle
 */
export function recordDefence(battle) {
    pending.push({
        attackerCountry: battle.attackerCountry,
        //Copied for the same reason `defenderCountry` is: the dice are drawn in the two
        //countries' colours, and asking the map for the attacker's colour at PLAYBACK time
        //would be asking about a world the battle has already changed.
        attackerColour: battle.attackerColour ?? null,
