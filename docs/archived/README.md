...
| [05-economy-audit.md](./05-economy-audit.md) | What the economy is, the five places it reaches the military and the dice, and the split between defects (E1-E7) and design (D1-D8) | Delivered, all four stages, and measured twice - once for the defect half and once for the tuning half. The living description is the Dominapedia and the economy gotchas in `CLAUDE.md`. Its section 5 -- the list of what is RIGHT and had to survive the phase -- still holds. **One thing it left open is now the current phase**: fixing the defects made the AI pay for its attacks, and the world stopped conquering. |
