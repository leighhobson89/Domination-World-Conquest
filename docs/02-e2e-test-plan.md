...
| `map-modes.spec.js` | The map-view button walks `continent → normal → military → physical` and wraps; the default is APPLIED and not merely declared; the relief map keeps the boundaries and the player's own colour; clicking the map while in physical mode reverts to political, and clicking it in the military view deliberately does NOT |
| `military-view.spec.js` | The military view collapses 207 political fills onto the five-step force ramp and restores them on the way out; the force figures are drawn while it is up and taken away with it; a player territory whose border becomes indefensible takes the threat outline. No colour is asserted — the ramp is two theme tokens, and which band a territory falls in is `tests/unit/ui-military-shading.spec.js` |
