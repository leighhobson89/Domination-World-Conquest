...
| **BK** | **An AI country's economy upgrades raise no capacity at all.** `analyzeAllocatedResourcesAndPrioritizeUpgradesThenBuild()` debits gold and cons. mats. and increments `farmsBuilt` / `forestsBuilt` / `oilWellsBuilt`, and never writes `foodCapacity`, `consMatsCapacity` or `oilCapacity` — the only upgrade-driven writes to those three fields anywhere are in `addPlayerUpgrades()`, which only the player reaches. Every farm, forest and oil well the AI has ever built has been a pure cost, up a quadratic price ladder, against a ceiling that never moves. See [05-economy-audit.md](./05-economy-audit.md) §4 E1 | economy 1.4 |
| **BL** | **An AI country's forts raise no defence bonus**, so an AI fort never moves the die band forts exist to move. Same function family: `analyzeAndBuildFortDefenses()` never recomputes `defenseBonus`. Three further loop errors beside it — the price is not recalculated between forts, `consMatsToSpend` is never decremented, and `fortsBuilt` is incremented AFTER the loop so the `maxForts` cap can be exceeded in one turn. The AI's STARTING forts are fine; they go through `addRandomFortsToAllNonPlayerTerritories()`. §4 E2 | economy 1.5 |
| **BM** | **The AI buys its main tranche of infantry at a tenth of the price.** `bolsterArmy()` debits `finalInfantryQuantity` — a troop COUNT — where it means the gold cost, so the AI gets 1,000 infantry per gold on that purchase where the player gets 100. It is the LAST tranche, after vehicles, so on most turns it is the bulk of the budget; the first tranche a few lines above is charged correctly, which is why it reads as fine. Check against **BJ** when fixing. §4 E3 | economy 1.10 |
| **BN** | **The upgrade price shown and the upgrade price charged come from different formulas.** `calculateAvailableUpgrades()` prices at the n=1 rate with no `n²` term, floored by `simulatedCostsAll` — a module-level array filled by the PREVIOUSLY rendered upgrade table and never reset, and read before the loop that fills it. So the "Can Build" label and the plus button's enabled state are decided from a price that is neither this territory's nor this building's. Six copies of the price formula exist and this is the one that disagrees. §4 E4, E5 | economy 1.7 |
