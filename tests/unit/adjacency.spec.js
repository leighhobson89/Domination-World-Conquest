...
// A ONE-WAY BORDER IS THE ONE THING THIS MAP MUST NEVER CONTAIN, and it has no
// signature at all: nothing throws, both countries plan normally, and the only
// witness is a territory that is attacked from a neighbour it can never attack
// back. It also skews the threat model, because `strongestEnemyPowerAgainst()`
// sizes a garrison from what can reach a territory -- a territory on the receiving
// end of a one-way link does not count the enemy that can reach it, so it
// under-garrisons against the only neighbour that can come.
//
// Both halves are asserted, because the two sources fail differently. The GEOMETRY
// is generated by tools/build-adjacency.mjs and has always been symmetric; the
// hand-curated table is where five one-way straits lived for as long as it has
// existed. Asserting only the table would miss a future generator change, and
// asserting only the geometry would have missed the five.
describe("adjacency symmetry", () => {
    ...
    it("has no one-way edge in the raw geometry", () => {
        const oneWay = [];
        for (const id of adjacencyIds()) {
            for (const name of getReachableFrom(id)) {
                const otherId = nameToId[name];
                if (otherId === undefined || !getReachableFrom(otherId).includes(idToName[id])) {
                    oneWay.push(`${idToName[id]} -> ${name}`);
                }
            }
        }
    ...
    // The one the game actually reads: geometry, plus the manual additions, minus
    // the manual denials. This is what froze at five one-way straits in the south
    // Pacific until the manual table's typos were repaired.
    it("has no one-way edge in the adjacency the game reads", () => {
        const oneWay = [];
        for (const id of adjacencyIds()) {
            const name = idToName[id];
            for (const neighbour of getInteractableFrom(id, name)) {
                const otherId = nameToId[neighbour];
                if (otherId === undefined ||
                    !getInteractableFrom(otherId, neighbour).includes(name)) {
                    oneWay.push(`${name} -> ${neighbour}`);
                }
            }
        }
        expect(oneWay).toEqual([]);
